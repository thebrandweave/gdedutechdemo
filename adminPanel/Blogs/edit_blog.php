...
// Load existing blog
$blog_q = $conn->prepare("SELECT * FROM Blogs WHERE blog_id = ?");
$blog_q->bind_param('i', $id);
$blog_q->execute();
$blog = $blog_q->get_result()->fetch_assoc();
$categories = [];
$catsRes = $conn->query("SELECT category_id, name FROM BlogCategories ORDER BY name ASC");
if ($catsRes) { while ($c = $catsRes->fetch_assoc()) { $categories[] = $c; } }

// Load existing social links
$socialQuery = $conn->prepare("SELECT * FROM social_links WHERE target_type = 'blog' AND target_id = ? ORDER BY platform ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['create_category'])) {
	...
	$coverFileName = $blog['main_cover_image'];
	$socialLinks = [];

    // Handle cover image upload (optional replacement)
	...
	if (!$error) {
		$stmt = $conn->prepare("UPDATE Blogs SET main_cover_image = ?, title = ?, content = ?, status = ?, category_id = NULLIF(?, 0) WHERE blog_id = ?");
		$stmt->bind_param('ssssii', $coverFileName, $title, $content, $status, $category_id, $id);
        if ($stmt->execute()) {
            // Update sections: strategy - delete and reinsert (simple, consistent with CASCADE)
            $conn->query("DELETE FROM BlogSections WHERE blog_id = " . intval($id));
            ...
            if (is_array($sectionTitles)) {
                for ($i = 0; $i < count($sectionTitles); $i++) {
                    ...
                    if ($st !== '' || $sc !== '' || $imgName) {
                        ...
                        $ins->bind_param('isssi', $id, $st, $sc, $imgName, $so);
                        $ins->execute();
                    }
                }
            }
            ...
            $deleteSocialStmt = $conn->prepare("DELETE FROM social_links WHERE target_type = 'blog' AND target_id = ?");
...
<head>
	...
	<title>Edit Blog - GD Edu Tech</title>
            ...
            <div class="col py-3">
                <div class="container-fluid">
                    <div class="row mb-4">
                        <div class="col">
                            <h2>Edit Blog</h2>
                            <p class="text-muted">Update blog details and content</p>
                        </div>
                        <div class="col-auto">
                    ...
                    <form method="post" enctype="multipart/form-data" class="card p-3">
                        <div class="mb-3">
                            ...
                            <div class="dropdown" data-bs-auto-close="outside">
                                <?php 
                                    $selectedId = $createdCategoryId ?: intval($blog['category_id'] ?? 0);
                                    $selectedCatName = '-- No category --';
                                    foreach ($categories as $cat) {
                                        if (intval($cat['category_id']) === intval($selectedId)) { $selectedCatName = $cat['name']; break; }
                                    }
                                ?>
                                ...
                                <input type="hidden" name="category_id" id="categoryIdInput" value="<?php echo intval($selectedId); ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <?php if (!empty($blog['main_cover_image'])): ?>
                                <div class="mt-2">
                                    <img src="<?php echo '../../uploads/blogs/' . htmlspecialchars($blog['main_cover_image']); ?>" alt="cover" height="100">
                                </div>
                            <?php endif; ?>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Content</label>
                            <textarea name="content" class="form-control" rows="8"><?php echo htmlspecialchars($blog['content']); ?></textarea>
                        </div>
                        <hr>
                        ...
                        <div id="sections-wrapper">
                            <?php
                                ...
                                $sectionsRes = $conn->query("SELECT * FROM BlogSections WHERE blog_id = " . intval($id) . " ORDER BY section_order ASC, section_id ASC");
                                if ($sectionsRes && $sectionsRes->num_rows > 0):
                                    while ($sec = $sectionsRes->fetch_assoc()):
                            ?>
                            <div class="section-item border rounded p-3 mb-3">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <label class="form-label">Section Title</label>
                                        <input type="text" name="section_title[]" class="form-control" value="<?php echo htmlspecialchars($sec['title']); ?>">
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label">Order</label>
                                        <input type="number" name="section_order[]" class="form-control" value="<?php echo intval($sec['section_order']); ?>">
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <?php if (!empty($sec['image'])): ?>
                                            <div class="mt-2">
                                                <img src="<?php echo '../../uploads/blogs/sections/' . htmlspecialchars($sec['image']); ?>" alt="section" height="70">
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label">Section Content</label>
                                        <textarea name="section_content[]" class="form-control" rows="4"><?php echo htmlspecialchars($sec['content']); ?></textarea>
                                    </div>
            ...
            if (addBtn) {
                addBtn.addEventListener('click', function(){
                    ...
                    clone.querySelectorAll('input').forEach(i => { 
                        if(i.type==='number' && i.name === 'section_order[]'){ 
                            // Find the highest order value currently in use
                            let maxOrder = 0;
                            wrapper.querySelectorAll('.section-item input[name="section_order[]"]').forEach(input => {
                                const orderValue = parseInt(input.value) || 0;
                                if (orderValue > maxOrder) {
                                    maxOrder = orderValue;
                                }
                            });
                            // Set the new section's order to one higher than the current maximum
                            i.value = maxOrder + 1;
