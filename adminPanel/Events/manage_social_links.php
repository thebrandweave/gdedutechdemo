...
if (!in_array($targetType, ['blog', 'event', 'website']) || ($targetType !== 'website' && $targetId <= 0)) {
    header('Location: ../');
    exit();
}
...
$targetTitle = '';
if ($targetType === 'blog') {
    $query = $conn->prepare("SELECT title FROM Blogs WHERE blog_id = ?");
    $query->bind_param('i', $targetId);
    $query->execute();
    $result = $query->get_result()->fetch_assoc();
    $targetTitle = $result ? $result['title'] : 'Unknown Blog';
} elseif ($targetType === 'event') {
    $query = $conn->prepare("SELECT title FROM Events WHERE event_id = ?");
    $query->bind_param('i', $targetId);
    $query->execute();
    $result = $query->get_result()->fetch_assoc();
    $targetTitle = $result ? $result['title'] : 'Unknown Event';
} else {
    $targetTitle = 'Website';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_link'])) {
    ...
    if ($platform && $url) {
        $insertQuery = $conn->prepare("INSERT INTO social_links (target_type, target_id, platform, url) VALUES (?, ?, ?, ?)");
        $targetIdForInsert = $targetType === 'website' ? null : $targetId;
        $insertQuery->bind_param('siis', $targetType, $targetIdForInsert, $platform, $url);
...
$whereClause = $targetType === 'website' ? "target_type = 'website' AND target_id IS NULL" : "target_type = ? AND target_id = ?";
$linksQuery = $conn->prepare("SELECT * FROM social_links WHERE $whereClause ORDER BY platform ASC");
if ($targetType === 'website') {
    $linksQuery->execute();
} else {
    $linksQuery->bind_param('si', $targetType, $targetId);
    $linksQuery->execute();
}
$socialLinks = $linksQuery->get_result()->fetch_all(MYSQLI_ASSOC);

// Common platforms
$platforms = ['facebook', 'twitter', 'instagram', 'linkedin', 'youtube', 'snapchat', 'discord', 'telegram', 'whatsapp'];
?>
...
<head>
    ...
    <title>Manage Social Links - GD Edu Tech</title>
            ...
            <div class="container-fluid">
                <div class="row mb-4">
                    <div class="col">
                        ...
                        <p class="text-muted">Target: <?php echo htmlspecialchars($targetTitle); ?></p>
                    </div>
                    <div class="col-auto">
                        <?php if ($targetType === 'event'): ?>
                            <a href="./" class="btn btn-outline-primary"><i class="bi bi-arrow-left me-2"></i>Back to Events</a>
                        <?php elseif ($targetType === 'blog'): ?>
                            <a href="../Blogs/" class="btn btn-outline-primary"><i class="bi bi-arrow-left me-2"></i>Back to Blogs</a>
                        <?php else: ?>
                            <a href="../" class="btn btn-outline-primary"><i class="bi bi-arrow-left me-2"></i>Back to Dashboard</a>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="bi bi-link-45deg me-2"></i>Social Links (<?php echo count($socialLinks); ?>)</h5>
                    </div>
                    <div class="card-body">
                                    ...
                                    <tbody>
                                        <?php foreach ($socialLinks as $link): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="?type=<?php echo $targetType; ?>&id=<?php echo $targetId; ?>&delete=<?php echo $link['id']; ?>" 
